...
*			该设备的寄存器。在某些情况下临界区的操作比较简单，并且要
*			求不能进入睡眠模式，这时需要使用spinlock
================================================================*/
...
关键词：
	...
	int write_func(){
		//访问临界区前抢锁
		if(spin_lock(&mspin_lock)){
		return -ERESTARTSYS;
		}
		
		//####################读写操作###############
		//临界区访问结束解锁
		spin_unlock(&sping_lock);
	}

	//屏蔽中断的加锁/释放锁
	int write_func(){
		//访问临界区前抢锁
		if(spin_lock_irqsave(&mspin_lock)){
		return -ERESTARTSYS;
		}
		
		//####################读写操作###############
		//临界区访问结束解锁
		spin_unlock_irqrestore(&sping_lock);
	}
